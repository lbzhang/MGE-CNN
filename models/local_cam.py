...
def get_mask(conv5, layer_weights, mask=None, rate=0.8):
    ...
    mask_sum = mask_sum.view(mask_sum.size(0), 1, 448, 448)
    if mask is None:
...
def get_bbox(x, conv5, layer_weights, rate=0.3):
    conv5_cam = conv5.clone().detach().view(conv5.size(0), conv5.size(1), 14*14) * layer_weights.unsqueeze(-1)
    mask_sum = conv5_cam.sum(1, keepdim=True).view(conv5.size(0), 1, 14, 14)
    mask_sum = F.interpolate(mask_sum, size=(448, 448), mode='bilinear', align_corners=True)
    mask_sum = mask_sum.view(mask_sum.size(0), -1)
...
class LocalCamNet(nn.Module):
    def __init__(self, opt=None):
        ...
        self.classifier_box = Classifier(2048, num_classes, bias=True)

        self.conv4_box_2 = copy.deepcopy(self.conv4)
        self.conv5_box_2 = copy.deepcopy(self.conv5)

        self.classifier_box_2 = Classifier(2048, num_classes, bias=True)

        # part information
        self.conv6_1 = nn.Conv2d(1024, 10*num_classes, 1, 1, 1)
        self.conv6_2 = nn.Conv2d(1024, 10*num_classes, 1, 1, 1)
        self.conv6 = nn.Conv2d(1024, 10*num_classes, 1, 1, 1)

        self.cls_part_1 = Classifier(10*num_classes, num_classes, bias=True)
        self.cls_part_2 = Classifier(10*num_classes, num_classes, bias=True)
        self.cls_part = Classifier(10*num_classes, num_classes, bias=True)

        self.cls_cat_1 = Classifier(2048+10*num_classes, num_classes, bias=True)
        self.cls_cat_2 = Classifier(2048+10*num_classes, num_classes, bias=True)
    ...
    def forward(self, x, y=None, is_vis=False, vis_idx=None, gt_top=None):
        ...
        # ---bbox---
        input_box, box_xy = get_bbox(x, conv5, layer_weights, rate=self.box_thred)
        ...
        input_box_2, box_xy_2 = get_bbox(input_box, conv5_box, layer_weights_2, rate=self.box_thred)
